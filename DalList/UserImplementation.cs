...
internal class UserImplementation : IUser
{
    public int Create(User item)//Adding a new engineer to the list if there is no engineer with the same ID.
    {
        ...
        if (DataSource.Engineers.Find(Engineer => Engineer.Id == item.EngineerId) == null)
            throw new DalDeletionImpossible($"Engineer with ID={item.EngineerId} dose not exist");
        DataSource.Users.Add(item);//Adding the new user to the list.
        return item.EngineerId;//Returning the user ID.
    }

    public void Delete(int id)//Deletion of a user with EngineerId=id if it exists and can be deleted.
    {
        User? toDel = DataSource.Users.Find(User => User.EngineerId == id);//Reference to the user with Id=id or null to if it doesn't exist.
        if (toDel == null)//user does not exist, error.
            throw new DalDeletionImpossible($"User with ID={id} dose not exist");
        if (DataSource.Tasks.Find(Task => Task.EngineerId == id) != null)//The user is assigned to the task, error.
    ...
    {
        return DataSource.Users.FirstOrDefault(User => User.EngineerId == id);//Searching for a user with EngineerId=id and returning the reference to it.
    }
    ...
    public void Update(User item)//Update of an existing user.
    {
        ...
        User? oldUser = DataSource.Users.Find(User => User.EngineerId == item.EngineerId);
        //There is no user that needs to be updated, so there is an exception:
        if (oldUser == null)
            throw new DalDoesNotExistException($"User with ID={item.EngineerId} dose not exist");
